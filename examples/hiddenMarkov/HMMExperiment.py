...
def enumerate_HMM(size,card,order):
    ...
    transition_cpts = enumerate_transitions9(size,card,order,threshold=MUTUAL_INFO_THRESHOLD,step_size=TRANSITION_STEP_SIZE)
    #transition_cpts = enumerate_one_transition()
    #random.shuffle(transition_cpts)
    for transition_cpt, mutual_info,group in transition_cpts:
        for emission_cpt in enumerate_sensors1(card):
        # enumerate transition cpt for node card with parents cards
            yield (transition_cpt, emission_cpt, mutual_info,group)
